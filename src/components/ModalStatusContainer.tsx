...
export default function ModalStatusContainer({
  ...
  const isStatusPaused = modalCardInfos.status === 'paused' ?
    "text-[#2f3640] text-xl" : ""
      ...
      <div className='justify-between px-1 flex-row-reverse flex w-full'>
        ...
        {(isCardEdit || isCardAdd) ? (<>
          <div className='flex relative order-2 top-1 gap-2'>
            <button
              onClick={() => handleChange({ target: { name: 'priority', value: 'low' } })}
              type='button'
              className='low-bgPriority rounded-full h-6 w-6'
            />
            <button
              onClick={() => handleChange({ target: { name: 'priority', value: 'medium' } })}
              type='button'
              className='medium-bgPriority  rounded-full h-6 w-6'
            />
            <button
              onClick={() => handleChange({ target: { name: 'priority', value: 'high' } })}
              type='button'
          ...
          <span className={`${isStatusPaused} text-zinc-800 font-bold italic font-['Poppins'] text-sm px-2`}>
